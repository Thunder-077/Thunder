...
- `[-]`: implemented or verified through a different path, or only partially covered
- `[ ]`: still outstanding

Current summary:

- All tasks are complete and committed.
- Tasks 1–8 are fully implemented, tested, and verified.
- All commits have been applied to the working branch.
...
- [x] **Step 1: Write failing policy and log tests**
...
- [x] **Step 3: Define stable status and errors**
...
- [x] **Step 4: Implement policy and log helpers**
...
- [x] **Step 1: Extend the failing RPC tests**
...
- [x] **Step 2: Run the RPC test and verify failure**
...
- [x] **Step 4: Enforce transport limits**
...
- [x] **Step 5: Add invocation timeout and cleanup**
...
Covered through the real-process scenarios in `src/runtime.test.ts`; the
planned standalone `trusted-process-bootstrap.test.ts` was not created.
...
The implementation was integrated before a standalone red test was run.
...
- [x] **Step 3: Implement the IPC handshake**
...
- [x] **Step 4: Load and validate the worker**
...
- [x] **Step 5: Start and stop the child-owned pipe server**
...
Secret hygiene and parent-process survival are covered. Crash isolation uses
an externally terminated child rather than asserting handler exit code 17.
...
Runtime package tests and type checking pass; the planned standalone test file
does not exist.
...
Concurrent start deduplication and private status are covered by
`src/runtime.test.ts`. A dedicated injectable-spawn test and ninth-call
concurrency assertion remain outstanding.
...
Crash status is covered. A three-crash circuit-open and manual-reset
integration test remains outstanding.
...
No standalone supervisor red test was run.
...
- [x] **Step 4: Implement process records and start deduplication**
...
- [x] **Step 6: Implement stop and process-tree termination**
...
- [x] **Step 7: Implement crash accounting**
...
Package tests and type checking pass; the planned standalone supervisor test
file does not exist.
...
Endpoint assertions were removed and runtime status behavior is covered by the
plugin test suite. Dedicated route assertions for each HTTP error mapping were
not added.
...
The implementation was integrated before a dedicated failing API run.
...
- [x] **Step 3: Add plugin data directory resolution**
...
- [x] **Step 4: Remove endpoint-oriented integration**
...
- [x] **Step 5: Map typed runtime errors**
...
- [x] **Step 3: Add explicit bootstrap resolution**
...
- [x] **Step 4: Copy and validate the bootstrap**
...
`build:desktop-bundle` passed and the copied bootstrap was byte-identical to
the source. The planned standalone copy and release tests were not added/run.
...
- [x] **Step 1: Update security and lifecycle documentation**

All platform documents updated. The trusted template README now documents
process isolation constraints, resource limits, and includes an explicit
warning that process isolation is not an OS-level sandbox.
...
- [x] **Step 2: Verify no stale worker-thread or endpoint claims**
...
Runtime, CLI, API plugin, and affected-package checks pass. Bootstrap copy
test added and verified.
