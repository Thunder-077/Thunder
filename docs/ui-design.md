...
- 内容区使用 `AppShell` 统一承载。
- Tauri 桌面端使用自绘标题栏，默认高度约 `56-57px`，左侧品牌与上下文，右侧窗口控制按钮；禁止继续使用“网页右上角浮动工具组”冒充桌面标题栏。
- 普通页面顶部使用统一 `PageHeader` 结构：左侧标题/描述，右侧操作区。
- 顶部个人区不再使用独立 Topbar 整行占位，默认作为页面头部右侧操作区，与标题共同组成统一 Header。
- 主内容容器默认接近全宽展示，使用统一页面边距控制留白：移动端 `16px`，桌面端 `24px`，超宽屏 `32px`。
- 页面默认内边距：
  - 移动端 `px-4 py-5`
  - 桌面端 `px-6 py-5`
- 页面底部的辅助文案或 footer 信息应跟随主内容流自然结束，避免做成独立底栏遮挡页面内容或形成背景割裂。
...
- 页面标题区默认带底部分隔线。
- 头部使用统一横向栅格：左侧标题/描述，右侧操作区或页面 actions。
- 标题上方可显示小型 section label。
- 标题、描述、右侧 actions 保持一个统一结构，不在页面里各自重写。
- 分隔线保持克制，优先使用 `border-border/60` 或更弱，不做完整横条 Topbar。
- 右侧操作区默认表现为标题右侧的轻量工具栏，不使用明显胶囊包裹，不与标题形成割裂卡片感。

### 顶部个人区规范

- 内容区右上角显示后端用户表中的当前登录用户头像；没有头像时显示用户名首字母。
- 普通页面中，个人区作为 `PageHeader` 右侧操作区的一部分出现，不单独漂浮在 viewport 上。
- 沉浸式页面中，个人区仍需放在 content shell 内部对齐展示，不能固定到浏览器右上角。
- 个人区优先融入 Header 背景，表现为行内工具组；仅在必要时使用极弱背景或分隔线，不做明显胶囊化。
- 用户菜单首项展示头像和用户名，点击头像可修改头像并保存到后端用户表。
- 上传头像后必须先进入裁剪弹窗，使用固定圆形裁剪区；通过拖动图片调整位置、滚轮缩放，不使用显眼的缩放拖拽条。
- 头像展示使用弱 ring 边框，图片内部圆形裁切并保持高清缩放。
- 用户菜单底部固定放置退出登录入口，使用 destructive 语义。
- 菜单项保持 Thunder 自身功能语义，不照搬第三方产品菜单结构。

### 登录页规范

- 登录页 Web 端不套用 AppShell、Sidebar 和 Topbar；Tauri 桌面端仍保留自绘标题栏，确保窗口拖拽与关闭控制可用。
- 登录页在桌面端使用全屏双栏布局，左侧插画融入背景，右侧使用高登录面板，不使用居中浮动登录卡片。
- 登录页允许使用品牌图标和 Storyset 风格插画，插画主色跟随品牌浅蓝 `#90CAF9`，不展示敏感配置或调试信息。
- 登录表单复用通用 Input、PasswordInput 和 Button。
- 登录失败只展示必要错误，不暴露服务端内部细节。
...
- 启动 Splash 使用白底全屏覆盖 + 居中品牌图标，先轻入场，再进入对角线块状消失/出现的克制循环。
- 路由级 loading 复用品牌图标，只保留轻量块状循环，不使用扫光或强闪。
