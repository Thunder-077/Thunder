...
- 报价分享页：生成面向客户的“窗帘报价单”图片，包含客户、电话、地址、日期、房间宽度、房间金额、总价、优惠和最终报价，不展示温馨提示、布价、轨道、衬带、环/勾等过程信息。
- 保存图片：分享页通过 Canvas 生成报价单图片，并调用小程序相册保存能力；页面不再提供单独的图片预览按钮。
...
P0 页面不直接裁切 UI 设计稿截图作为素材。首页场景和汇总页房间缩略图使用压缩后的本地真实家居图片，房间缩略图统一使用 200K 以下 JPG，当前包含 `客厅`、`卧室`、`书房`、`通用房间` 等素材映射；报价方式插画保留本地 SVG。

当前暂不启用 `lazyCodeLoading: "requiredComponents"`：Taro 4.2 生成的微信端根组件产物在开发者工具中会触发 `wx://not-found` 组件解析错误。新增图片、音频等静态资源时，单文件应优先控制在 200K 以下；超过 200K 的非必要资源应压缩、改用远程 URL，或拆分到更合适的加载路径。
